...
const mockData = {
  ...
  mapPins: [{
      ...
      "description": "Farmacia abierta las 24 horas en Avenida Providencia cerca del Costanera Center. Muy útil para emergencias y medicamentos.",
      "tags": [
          "farmacia",
          "24 horas",
          "emergencias",
      ...
      "background_image": "https://images.unsplash.com/photo-1587854692152-cbe660dbde88?w=800",
      "location": {
          "point": {
              "type": "Point",
              "coordinates": [
      ...
      "comments": [],
      "likedBy": [
          "69223cb2b6ded3ffe0399b11",
          "69223cb2b6ded3ffe0399b13"
      ],
      "dislikedBy": [],
...
export default function TipIdentifierPage({ params }: { params: { identifier: string } }) {
  ...
  return (
    <div className="min-h-screen bg-[var(--background)]">
      ...
      <div className="relative w-full h-48 md:h-64 flex flex-col items-start justify-end rounded-b-lg" style={{
        ...
        {data.portadaUrl ? (
          ...
          <div className="absolute top-0 left-0 w-full h-full rounded-b-lg" style={{ background: 'var(--color-primary)', border: '1.5px solid rgb(0, 0, 0)' }} />
        )}
        <div className="absolute left-4 bottom-[-80px] flex items-end">
          <div className="w-28 h-28 md:w-40 md:h-40 rounded-full bg-white border-0 border-[var(--background)] shadow-lg overflow-hidden relative z-10"
          style={{
          ...
          >
            {data.avatarUrl ? (
              <Image src={data.avatarUrl} alt="Avatar" width={160} height={160} className="w-full h-full object-cover" />
            ) : (
              <User className="w-full h-full" style={{ color: 'var(--foreground)' }} />
            )}
          </div>
          <div className="ml-6 mt-6 flex flex-col">
            <h1 className="text-lg md:text-3xl font-bold text-[var(--foreground)] drop-shadow-sm">{data.title}</h1>
            <button
              onClick={() => setJoined((prev) => !prev)}
              className={
                `mt-2 px-2 py-0.5 rounded-full text-xs font-medium transition-all w-fit ` +
                (joined ? 'bg-[var(--color-chip-3)]' : 'bg-[var(--color-chip-4)]')
              }
              style={{
      ...
      <div className="px-4 md:px-8 py-2">
        <p className="text-[var(--foreground)] text-base whitespace-pre-line">{data.description}</p>
      </div>
      {/* Map section */}
      <div className="px-4 md:px-8 py-2">
        {/* <Map markers={mockData.mapPins[0]} onChangeCenter={() => {}} /> */}
      </div>
      {/* Discussions section */}
      <div className="px-4 md:px-8 py-4">
        <div className="flex items-center justify-between mb-2">
          <h2 className="text-lg font-semibold text-[var(--foreground)]">Discussions</h2>
          <button
            className={
              `px-4 py-2 rounded-full text-sm font-medium transition-all w-fit bg-[var(--color-chip-3)]`
            }
            style={{
              color: 'var(--foreground)',
              fontFamily: 'Helvetica Neue, Helvetica, Arial, sans-serif',
