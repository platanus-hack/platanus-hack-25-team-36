# Pasa el dato

Conecta con lo que tu barrio tiene para ofrecerte. Pasa el dato es una plataforma que te permite descubir eventos, servicios y tips locales, de manera inteligente y personalizada.

<img src="./project-logo.png" alt="Project Logo" width="200" />

Track: Legacy

Team-46

- Felipe Snahueza ([@Sanhuezafce](https://github.com/Sanhuezafce))
- Gabriela Mendoza ([@gabrielaelisa](https://github.com/gabrielaelisa))
- Pedro Belmonte ([@pbelmonte](https://github.com/pbelmonte))
- Víctor Garrido ([@victorPato](https://github.com/VictorPato))
- Maximiliano Vargas ([@mvargas33](https://github.com/mvargas33))
